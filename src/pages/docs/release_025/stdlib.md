# bpftrace Standard Library (0.25)
...
performance and correctness using cheap, thread-safe writes ([PERCPU](./language#percpu-types)). However, sync reads
...
Similar to `count` this uses a [PERCPU](./language#percpu-types) map (thread-safe, fast writes, slow reads).
